...
const totTokenArtifacts = require('../build/contracts/TOTToken.json');
const contract = require('truffle-contract');
let TotToken = contract(totTokenArtifacts);
...
TotToken.setProvider(web3.currentProvider);

let totTokenAddress = process.argv.slice(2)[0];
...
async function setAllocation () {
  ...
  let totToken = await TotToken.at(totTokenAddress);
  ...
  for (var i = 0; i < distribAddressData.length; i++) {
    try {
      ...
      let r = await totToken.batch(distribAddressData[i], distribAmountData[i], { from: walletProvider.getAddress(), gas: 4500000, gasPrice: 2000000000 });
      console.log('---------- ---------- ---------- ----------');
...
if (totTokenAddress) {
  console.log('Processing TOTToken allocation. Batch size is', BATCH_SIZE, 'accounts per transaction');
  readFile();
} else {
  console.log('Please run the script by providing the address of the TOTToken contract');
}